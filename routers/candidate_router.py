...
async def update_candidate_status(
    ...
    async with session.begin():
        ...
        try:
            current_idx = status_flow.index(candidate.status)#获取候选人当前状态在列表中的位置
            target_idx = status_flow.index(status_data.status)#获取要更新的目标状态在列表中的位置
        except ValueError:
