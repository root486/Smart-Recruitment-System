...
async def update_status(
        status_data:UserStatusUpdateSchema,#修改员工状态需要传入的数据
        session:AsyncSession=Depends(get_session_instance),
...
async def dingtalk_authorize_success(
    ...
    return jinja2Engine.TemplateResponse(
        request=request,  # 明确指定 request 参数
        name="ding_authorize_success.html",  # 明确指定模板名称
        context={"username": nick}  # 上下文字典只包含业务数据
    )
